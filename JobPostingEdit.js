...
const JobPostingEdit = () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setFormData({
          ...
          state: job.state || '',
          salary: job.salary || '',
          bonusInfo: job.bonusInfo || '',
          degreeRequired: job.degreeRequired || '',
          allowedBranches: job.allowedBranches || [],
          graduationYear: job.graduationYear || '',
          minCGPA: job.minCGPA ? String(job.minCGPA) : '',
          requiredSkills: job.requiredSkills || [],
          certifications: job.certifications || [],
          backlogAllowed: job.backlogAllowed || false,
          selectionProcess: {
            prePlacementTalk: job.selectionProcess?.prePlacementTalk || false,
            writtenTest: job.selectionProcess?.writtenTest || false,
            groupDiscussion: job.selectionProcess?.groupDiscussion || false,
            technicalInterview: job.selectionProcess?.technicalInterview || false,
            hrInterview: job.selectionProcess?.hrInterview || false
          },
          numberOfPositions: job.numberOfPositions ? String(job.numberOfPositions) : '',
          applicationDeadline: job.applicationDeadline ? new Date(job.applicationDeadline).toISOString().substring(0,10) : '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/company/job-postings/${id}`, formData, {
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      if (response.status === 200) {
        ...
        navigate(`/company/job-postings/${id}`);
      }
    } catch (error) {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>Edit Job Posting</h1>
        <div style={styles.headerActions}>
          <button 
            style={styles.backButton}
            onClick={() => navigate(`/company/job-postings/${id}`)}
          >
            ← Back to View
          </button>
          <button 
