...
// Student view navigates to a dedicated details page instead of using company view
...
const StudentJobPostingsView = () => {
  const navigate = useNavigate();
  const [jobPostings, setJobPostings] = useState([]);
  const [loading, setLoading] = useState(true);
  // We navigate to the student job details route on view
  ...
  const handleViewJob = (jobId) => {
    navigate(`/student-job-postings/${jobId}`);
  };

  // No inline modal view; details are shown on a dedicated route
  ...
  // Detailed view is handled by route `/student-job-postings/:id`
    ...
    <div style={styles.container}>
      ...
      <div style={styles.mainContent}>
        <div style={styles.pageHeader}>
          <h1 style={styles.pageTitle}>Job & Internship Opportunities</h1>
        </div>
            ...
            {jobPostings.map(job => (
              <div key={job._id} style={styles.jobCard}>
                ...
                <div style={styles.jobActions}>
                  ...
                  >
                    {new Date(job.applicationDeadline) < new Date() ? 'Expired' : 'Apply Now'}
                  </button>
                </div>
              </div>
            ))}
...
export default StudentJobPostingsView;
